...
public class RadarDbHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + UserEntry.TABLE_USER + " (" +
                ...
                "PRIMARY KEY( " + UserEntry.COLUMN_USER_ACCOUNT + "," + UserEntry.COLUMN_USER_USED_FOR + ") ) "
        );

        db.execSQL("CREATE TABLE " + LocationEntry.TABLE_LOCATION + " (" +
                LocationEntry._ID + " INTEGER, " +
                LocationEntry.COLUMN_LOCATION_ACCOUNT + " TEXT NOT NULL, " +
                ...
                "PRIMARY KEY( " + LocationEntry.COLUMN_LOCATION_ACCOUNT + "," + LocationEntry.COLUMN_LOCATION_TIME + ") ) "
        );
    }
...
/*
    public void addUser(String account, String password, String usedFor, String in_use) {
        ...
        if (!c.moveToFirst()) {
            ...
            values.put(UserEntry.COLUMN_USER_ACCOUNT, account);
    ...
    public String searchNewTime(String account) {
        ...
        Cursor cursor = getReadableDatabase().query(
                LocationEntry.TABLE_LOCATION, new String[]{LocationEntry.COLUMN_LOCATION_TIME}, LocationEntry.COLUMN_LOCATION_TIME + ">?" + " AND " +
                        LocationEntry.COLUMN_LOCATION_ACCOUNT + "=?", new String[]{"1911-01-01-00:00:00", account},
    ...
    public List<SimpleLocation> searchAllLocation(String account) {
        ...
        Cursor cursor = getReadableDatabase().query(
                LocationEntry.TABLE_LOCATION, null, LocationEntry.COLUMN_LOCATION_ACCOUNT + "=?", new String[]{account},
                null, null, null);
        ...
        if (cursor.getCount() > 0) {
            ...
            while (!cursor.isAfterLast()) {
                ...
                int index_ac = cursor.getColumnIndex(LocationEntry.COLUMN_LOCATION_ACCOUNT);
...
//        String[] user = new String[2];
        ...
        if (cursor.getCount() > 0) {
            ...
            while (!cursor.isAfterLast()) {
                User user = new User();
                int index_account = cursor.getColumnIndex(UserEntry.COLUMN_USER_ACCOUNT);
